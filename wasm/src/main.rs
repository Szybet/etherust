...
use good_web_game::graphics::LineCap;
use good_web_game::graphics::LineJoin;
use good_web_game::graphics::PxScale;
...
use good_web_game::input::mouse::button_pressed;
use good_web_game::input::mouse::cursor_type;
use good_web_game::input::mouse::position;
use good_web_game::input::mouse::set_cursor_type;
use rand::prelude::*;
...
impl MainState {
    fn new(ctx: &mut Context) -> GameResult<MainState> {
        let main = graphics::Image::from_png_bytes(ctx, include_bytes!("../resources/Main.png"))?;
        let true_image =
            graphics::Image::from_png_bytes(ctx, include_bytes!("../resources/true.png"))?;
        let false_image =
            graphics::Image::from_png_bytes(ctx, include_bytes!("../resources/false.png"))?;
        let font =
...
impl event::EventHandler<good_web_game::GameError> for MainState {
    fn update(&mut self, _ctx: &mut Context) -> GameResult {
        Ok(())
    }

    fn resize_event(&mut self, mut ctx: &mut Context, width: f32, height: f32) {
        if width != 1280.0 {
            self.requested_text.push(2);
        }
        if height != 800.0 {
            self.requested_text.push(2);
        }
...
pub fn manage_all_buttons(
    ...
    for mut button in vector {
        ...
        if cursor_type(ctx) == miniquad::CursorIcon::Crosshair {
        } else {
            if button.clicked_frames == 0 {
...
pub fn manage_objects(
    ...
    if is_overlapping == true {
        if button_pressed(ctx, MouseButton::Left) == true {
            set_cursor_type(ctx, miniquad::CursorIcon::Crosshair);
        } else {
            set_cursor_type(ctx, miniquad::CursorIcon::Pointer);
            *object_grabbed = 1000;
        }
    } else {
        set_cursor_type(ctx, miniquad::CursorIcon::Default);
        *object_grabbed = 1000;
    }
}
...
pub fn grab_overlaps_connect(
    ...
    if object_grabbed == 1000 {
        if connected == &mut 1000 {
            if cursor_type(ctx) == miniquad::CursorIcon::Pointer {
...
pub fn main() -> GameResult {
    ...
    */
    /// well https://github.com/ggez/good-web-game
    ///
    //let cb = good_web_game::Context::new("etherust", "szybet")
    ...
    ggez::start(
        ggez::conf::Conf::default()
            .window_title("etherust".to_string())
            .window_width(1280)
            .window_height(800)
        ...
        |mut ctx| {
            ...
            let mut settings_button = static_rect_button {
                ...
                button_image: graphics::Image::from_png_bytes(
                    &mut ctx,
                    include_bytes!("../resources/settings.png"),
                )
                .unwrap(),
                button_image_clicked: graphics::Image::from_png_bytes(
                    &mut ctx,
...
pub fn declare_variables(
    ...
    let mut location: Point2 = Point2::from([400.0, 300.0]);
    ...
    for num in 0..8 {
        if state_static_objects_to_delete.contains(&num) {
            ...
            let mut rec = static_rect_data {
                ...
                object_id: num,
                texture: graphics::Image::from_png_bytes(ctx, cable_file).unwrap(),
            };
    ...
    let mut button_reset = static_rect_button {
        ...
        button_image: graphics::Image::from_png_bytes(
            ctx,
            include_bytes!("../resources/Reset.png"),
        )
        .unwrap(),
        button_image_clicked: graphics::Image::from_png_bytes(
            ctx,
    ...
    let mut check_button = static_rect_button {
        ...
        button_image: graphics::Image::from_png_bytes(
            ctx,
            include_bytes!("../resources/Check.png"),
        )
        .unwrap(),
        button_image_clicked: graphics::Image::from_png_bytes(
            ctx,
    ...
    let mut resize_text = Text::new(format!("Reload the browser window"));
    resize_text.set_font(state.main_font, PxScale::from(120.0));

    let mut resize = text_time {
        text: resize_text,
        cord: Vec2::new(100.0, 200.0),
        color: Color::RED,
        text_id: 2,
